...
#else
				SpiSendRaw(fast_flip(active_buffer[lcd_width - 1 - a + (j * lcd_width)]));
#endif
			}
#ifndef STM32
			this->spi->UnsetSlave();
...
void lcd_display::Draw()
{
	setAddr(0x00 + 20, 0x00 + 20, lcd_width + 20 - 1, lcd_height + 20);//go home

	for (uint8_t j = 0;j < 6;j++) {
		for (uint8_t b = 0; b < 8; b++)
			for (uint8_t a = 0; a < lcd_width; a++) {
				if ((active_buffer[a + (j * lcd_width)]) & (1<<b))
					writedata16(0xffff);
				else
					writedata16(0);
		}
